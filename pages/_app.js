...
export default function App({ Component, pageProps }) {
  ...
  useEffect(() => {
    ...
    // 전역에 다시 호출해서 컴포넌트가 처음 로드될 때 스크롤 위치 확인해서 헤더 class 붙여서 출력
    handleScroll();
    ...
    // 컴포넌트가 언마운트(화면에서 사라짐)될 때 이벤트 리스너를 제거
    // 이유 : window에 이벤트 리스너를 추가하면, 그 컴포넌트가 화면에서 사라져도 window에 등록된 이벤트 리스너는 남아있어 불필요하게 메모리를 차지하거나 이벤트 핸들러가 계속 작동할 수 있음
  }, []); // -> 여기서 대괄호는 의존성 배열임 / useEffect에 의존성 배열을 추가하지 않으면 useEffect 안쪽 코드를 컴포넌트가 매번 렌더링될 때마다 실행시킴 -> 성능저하 올 수 있음

  // useEffect 를 쓰는 경우는 여러가지가 있는데 우리는 부수효과(side effect)를 처리할때 사용함
  // 좀 더 쉽게, "DOM 관련 작업"이나 "이벤트 등록 등 렌더링 외의 작업"을 처리할 때 꼭 필요한 곳에 사용
  // useEffect를 사용함으로써 초기 로드 시 스크롤 상태 확인과 이벤트 리스너 관리라는 부수 효과를 효율적이고 안전하게 처리할 수 있음
  // onClick 이벤트 같은 경우는 특정요소에 처리하지만 scroll 이벤트는 window 전체 스크롤을 감지해야하므로 useEffect와 window.addEventListener 사용

  // ---------------------------------------------------------------

  // 필터링 -> useState를 이용해 카테고리 상태를 _app.js에서 전역으로 관리
  // 상태를 배열로 설정
  const categories = ["전체", "가구", "조명", "홈데코", "아트"];
  const [selectedCategory, setSelectedCategory] = useState("전체"); // 선택된 카테고리만 상태로 관리
    ...
    <>
      <CardProvider>
        ...
        <header className={isScrolled ? "header_area scroll" : "header_area"}>
          {/* 기존 헤더 클래스명만 출력할건지 scroll 클래스명까지 같이 출력할건지 */}
          <div className="top">
            {/* <div className="logo">
            <img src="https://img.29cm.co.kr/next-contents/2023/06/08/3f8131682d124d16b336774ba51c4a3e_20230608162823.png" alt="" />
          </div> */}
            {/* Link 로 변경 */}
        ...
        {/* _app.js에서 관리 중인 selectedCategory 상태를 각 페이지 컴포넌트(Component)에 전달하기 위해 사용 */}
        {/* selectedCategory라는 이름의 props(컴포넌트 속성)로 현재의 selectedCategory 상태 값을 하위 컴포넌트에 넘겨준다는 의미 */}

        {/* CardContext 파일안에 있는 객체들로 서브페이지도 내려받게처리 */}
        <Component {...pageProps} selectedCategory={selectedCategory} />
